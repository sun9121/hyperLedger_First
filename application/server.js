...
const FabricCAServices = require('fabric-ca-client') // 1
    ...
    if(mode == 1){ // 관리자인증서
        ...
        try {
            ...
            if (adminExists) {
                ...
                // 오류전송 to 클라이언트
                const obj = JSON.parse('{"ERR_MSG":"An identitiy for the admin user admin already exists in the wallet"}')
            ...
            const obj = JSON.parse('{"MSG":"Successfully enrolled admin user admin and imported it into the wallet"}')
        ...
        try {
            ...
            if (!adminExists) {
                ...
                const obj = JSON.parse('{"ERR_MSG":"Run the enrollAdmin.js application before retrying"}')
                response.status(400).json(obj);;
            }
            ...
            console.log(`Successfully registered and enrolled ${id} and imported it into the wallet`);
            const obj = JSON.parse(`{"MSG":"Successfully registered and enrolled ${id} and imported it into the wallet"}`);
            response.status(200).json(obj);
...
app.get('/asset', async(request, response)=>{
    ...
    //const resultPath = path.join(process.cwd(), '/views/result.html')
    //var resultHTML = fs.readFileSync(resultPath, 'utf8');
    //resultHTML = resultHTML.replace("<div></div>", `<div><p>Transaction has been submitted: ${txresult}</p></div>`);
    //response.status(200).send(resultHTML);
    const obj = JSON.parse(txresult);
    response.status(200).json(obj);
...
app.get('/assets', async(request, response)=>{
    ...
    for (var i=0; i<txs.length; i++)
    {
        ...
        tableHTML+="<td>"+txs[i].TxId+"</td></tr>";
        tableHTML+="<tr><td>Timpstamp</td>";
        tableHTML+="<td>"+txs[i].Timestamp+"</td></tr>";
        tableHTML+="\n";
    }
    tableHTML+="</table>\n";
